...
# Load data
data = pd.read_csv('data/FINAL_agg_Dataset.csv')
...
# Statistically significant features according to causal analysis results (p < 0.05)
features = [
    'Contactwardofficer',
    'Informedlocal', 'Listentoconcerns', 'Reliedontobethere',
    'Treateveryonefairly', 'Understandissues', 'Gender_Male', 'Gender_Female',
    'Agerange_18-24', 'Agerange_25-34', 'Agerange_over34', 'Objectofsearch_EvidenceofoffencesundertheAct',
    'Objectofsearch_Stolengoods', 'violent_q_crimes', 'other_q_crimes'
]
...
for borough in boroughs:
    ...
    rf_trust.fit(borough_data[features][:'2019'], borough_data['TrustMPS'][:'2019'])
    ...
    rf_confidence.fit(borough_data[features][:'2019'], borough_data['GoodJoblocal'][:'2019'])
    ...
    sarima_trust_until_2019 = SARIMAX(train_data['TrustMPS'][train_data['borough'] == borough],
    ...
    sarima_confidence_until_2019 = SARIMAX(train_data['GoodJoblocal'][train_data['borough'] == borough],
                                           exog=train_data[features][train_data['borough'] == borough],
                                           order=(1, 1, 1),
                                           seasonal_order=(1, 1, 1, 4))
    ...
    sarima_trust_full = SARIMAX(borough_data['TrustMPS'],
                                exog=borough_data[features],
                                order=(1, 1, 1),
                                seasonal_order=(1, 1, 1, 4))
    sarima_trust_fit_full = sarima_trust_full.fit(disp=False)
    sarima_confidence_full = SARIMAX(borough_data['GoodJoblocal'],
                                     exog=borough_data[features],
                                     order=(1, 1, 1),
                                     seasonal_order=(1, 1, 1, 4))
    ...
    mse_trust = mean_squared_error(test_data['TrustMPS'][test_data['borough'] == borough], trust_predictions)
    mae_trust = mean_absolute_error(test_data['TrustMPS'][test_data['borough'] == borough], trust_predictions)
    rmse_trust = np.sqrt(mse_trust)

    mse_confidence = mean_squared_error(test_data['GoodJoblocal'][test_data['borough'] == borough],
                                        confidence_predictions)
    mae_confidence = mean_absolute_error(test_data['GoodJoblocal'][test_data['borough'] == borough],
                                         confidence_predictions)
    ...
    plt.plot(test_data.index[test_data['borough'] == borough], test_data['TrustMPS'][test_data['borough'] == borough],
    ...
    plt.plot(test_data.index[test_data['borough'] == borough],
             test_data['GoodJoblocal'][test_data['borough'] == borough], label='Actual "Good Job" Local')
    ...
    future_dates_until_2019 = pd.date_range(start='2020-01-01', periods=5, freq='Q')
    future_exog_until_2019 = pd.DataFrame(
        np.tile(train_data[features][train_data['borough'] == borough].iloc[-1].values,
                (len(future_dates_until_2019), 1)), columns=features)
    future_trust_predictions_until_2019 = sarima_trust_fit_until_2019.get_forecast(steps=len(future_dates_until_2019),
                                                                                   exog=future_exog_until_2019).predicted_mean
    future_confidence_predictions_until_2019 = sarima_confidence_fit_until_2019.get_forecast(
        steps=len(future_dates_until_2019), exog=future_exog_until_2019).predicted_mean

    # Predictions for future data using the full dataset
    future_dates_full = pd.date_range(start=data.index[-1], periods=3, freq='Q')[1:]
    future_exog_full = pd.DataFrame(np.tile(test_data[features].iloc[-1].values, (len(future_dates_full), 1)),
                                    columns=features)
    future_trust_predictions_full = sarima_trust_fit_full.get_forecast(steps=len(future_dates_full),
                                                                       exog=future_exog_full).predicted_mean
    future_confidence_predictions_full = sarima_confidence_fit_full.get_forecast(steps=len(future_dates_full),
                                                                                 exog=future_exog_full).predicted_mean
